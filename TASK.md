...
2. [F7.1](docs/tasks/active/F7.1.md): gate `READY` para a prova E2E vertical em wheel/repositório externo.
3. [F6.7](docs/tasks/completed/F6.7.md): produto PR #81/merge `93f7bf20`/CI `31977793119`; PR
   administrativo #82/head `cfd97c6`/CI `31978820506`/merge `38849ed`/pós-merge `31979153948`.
...
| **Estado corrente** | Lacuna vertical comprovada; baseline E2E vigente 16/16 verde; implementação ainda não iniciada |
...
| **Branch** | `task/f7.1-external-repo-e2e`, criada da base certificada `38849ed519436554b8f8c6b3e6950089a445a248` |
...
| Reconciliação administrativa | PR [#82](https://github.com/Wf-ops1/xXHarnessinfraXx/pull/82), final `cfd97c6`/CI `31978820506`; merge `38849ed`/pós-merge `31979153948` 11/11 |
| Fronteira | Fase 6 terminalmente encerrada; F7.1 ativa e F7.2 ainda proibida |
...
A [F7.1](docs/tasks/active/F7.1.md) está `READY / IMPLEMENTATION_AUTHORIZED`. O escopo inicial permite
somente a prova E2E e documentação; qualquer necessidade de mudar `src/` reabre o gate. F7.2 não iniciou.
`POST_PROMOTION_BLOCKED` permanece somente como autoridade histórica das correções anteriores.

## 6. Bloqueios e fronteiras externas

Não há bloqueio externo. A fronteira é o allowlist da F7.1: nenhum produto pode mudar sem reprodução
nova e recongelamento. Push/PR/merge seguem o lifecycle da tarefa; F7.2 permanece fora do escopo.

## 7. Próxima ação exata

```text
COMMITAR O GATE READY E CRIAR checkpoint/f7.1-ready SOMENTE LOCAL.
IMPLEMENTAR A PROVA VERTICAL SOMENTE EM tests/e2e/test_external_repository_product.py.
CERTIFICAR A F7.1 INTEGRALMENTE E PARAR ANTES DA F7.2.
```

## 8. Retomada após perda de contexto

1. Leia `.agents/AGENTS.md`, este painel, `docs/tasks/active/F7.1.md`, a Fase 7 e a DEC-014.
2. Confirme `task/f7.1-external-repo-e2e` sobre a base certificada `38849ed`; respeite o allowlist.
...
*Atualizado em: 2026-08-16T20:45:51-03:00 | Fonte: Fase 6 terminal + gate READY da F7.1*
