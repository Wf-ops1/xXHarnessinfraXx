...
> **Status da auditoria: Protótipo / Em desenvolvimento — F5.C1 promovida; reconciliação no PR #68**
...
| Evidência | `EvidenceManifestManager`, `RuntimeEngine` e audit trail | F6.2 valida/exporta o journal canônico; F6.3 local gera JSON estrito, agrega proveniência, ancora o evento terminal, verifica arquivos/digests e recupera publicação/CAS idempotentemente | F6.3 `COMPLETED_LOCAL / PROMOTION_PENDING`; efeito local real | Sem chave/âncora externa a proteção permanece somente tamper-evident local; CLI de verificação e recovery amplo pertencem à F6.5/F6.6 |
...
→ full, recuperação de cursor sem efeito duplicado e exaustão durável de todos os limites. Eles
  Para F5.4, também provam saldo idêntico após restart, reserva pré-efeito sem transporte/handler,
  uso real exatamente uma vez, tamper fail-closed e resume terminal sem novo efeito. Eles provam
  providers HTTP com servidores controlados,
