...
| Verificação | `VerificationEngine` | F4.5 promovida normaliza cinco IDs; F4.6 detecta configuração, resolve a suíte inteira e valida executáveis no `ProvisionedWorktree` antes de efeitos | F4.6 R3 `REPAIR_ACTIVE / PROMOTION_BLOCKED` | PR #44 precisa selecionar o launcher pelo `sys.prefix` do venv e preservar seu path no `TerminalAdapter`; depois da promoção, F4.7 persiste/guarda conclusão e F4.8 compõe retry |
