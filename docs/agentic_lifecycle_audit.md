...
| Ferramentas | `PolicyEngine`, `ToolRouter` e factory operacional | A F5.2 promovida avalia oito eixos com default-deny/deny-wins; a F5.3 local exige snapshot comum no router/adapters e revalida modo, raiz e capabilities antes do efeito | Primitiva real/injetável; F5.3 ainda local | Lifecycle padrão não constrói o registry nem injeta adapters; aprovação content-bound F5.6 ainda não está integrada |
...
| Promoção | `PromotionManager` + `ExecutionLifecycleService` | Candidate real no worktree, full suite no mesmo SHA, write-ahead/outcome, cherry-pick único, dry-run e recovery possuem E2E; F5.3 local acrescenta capability de promoção + aprovação antes do efeito | F3.7 `PROMOTED`; guard F5.3 local | Composição permanece opt-in; CLI/defaults não constroem manager/provider automaticamente |
