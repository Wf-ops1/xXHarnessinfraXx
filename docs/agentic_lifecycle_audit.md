...
| Contexto | `ContextAssembler` + `ExecutionLifecycleService` | Policy compilada, seis dimensões `Decimal`, dual gate, `context.json`, evento por digest, estados bloqueantes e resume possuem testes | F4.3 R6 `REPAIR_ACTIVE / PROMOTION_BLOCKED` | Auditoria pós-CI reproduziu suficiência sem evidência e com identidade divergente; a entrada também depende de artefatos e snapshot previamente produzidos; planner F4.4 não foi executado |
