...
| Verificação | `VerificationEngine` | F4.5 promovida normaliza cinco IDs; F4.6 detecta configuração, resolve a suíte inteira e valida executáveis no `ProvisionedWorktree` antes de efeitos; R2 preserva o launcher POSIX | F4.6 R2 `LOCAL_READY / PR_UPDATE_PENDING` | PR #44 aguarda novo head/CI; depois da promoção, F4.7 persiste/guarda conclusão e F4.8 compõe retry |
