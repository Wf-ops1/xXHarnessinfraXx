...
| Evidência | `RuntimeEngine` e audit trail | F6.2 local valida o journal canônico, detecta corrupção/gaps/duplicatas e exporta JSON/SARIF; checkpoint HMAC é opcional | Audit local implementado; evidence manifest ainda experimental | Sem chave/âncora externa a proteção é somente tamper-evident local; evidência F6.3 ainda não prova alteração entregue |
