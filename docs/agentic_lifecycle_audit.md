...
| Verificação | `VerificationEngine` | F4.5 promovida normaliza cinco IDs e bloqueia policy/suíte vazia, desconhecida, duplicada ou sem comando; F4.6 local detecta configuração, resolve a suíte inteira e valida executáveis no `ProvisionedWorktree` antes de efeitos | F4.6 `COMPLETED_LOCAL / PROMOTION_PENDING` | F4.7 persiste/guarda conclusão e corrige decisão da CLI; F4.8 compõe retry |
