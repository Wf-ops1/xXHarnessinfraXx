...
| Verificação e auditoria | F4.5 mantém a taxonomia única `typecheck/lint/unit_test/build/security_scan`; policy vazia/duplicada/desconhecida e runner `0/0` falham antes de subprocessos. A F4.6 promovida resolve a suíte no `ProvisionedWorktree`. A F4.7 concluída localmente persiste write-ahead/outcome e payloads commit-bound, recupera a suíte e impede `COMPLETED` sem ao menos um gate obrigatório executado e todos os obrigatórios aprovados | A branch F4.7 ainda aguarda publicação e CI; reprovação permanece em `VERIFYING` porque repair/reexecução pertencem à F4.8 | Matriz integral fail-closed com repair/recovery operacional |
...
- **Fase 4 iniciada:** F3.1–F3.6, F3.8 e as corretivas F3.C1/F3.C2 aplicáveis foram
  ...
  isolada verdes; a branch ainda não foi publicada. A F4.8 não foi iniciada e F3.7 continua
  dependente da promoção da F4.7.
...
- [ContextAssembler](src/ai_engineering_harness/runtime/context_assembler.py) calcula as seis dimensões
  ...
  worktree e preserva no terminal o launcher ativo selecionado por `sys.prefix`; a F4.7 concluída
  localmente persiste cada outcome e impede `COMPLETED` sem suíte obrigatória aprovada, mas sua
  publicação, CI e promoção ainda estão pendentes;
