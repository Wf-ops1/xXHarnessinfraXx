...
| Configuração e governança | F5.1–F5.7, F5.C1 e F6.1–F6.6 estão promovidas; a reconciliação F6.6 está `LOCAL_READY / PUBLICATION_PENDING` em `docs/promote-f6.6` | A transação knowledge permanece `known_gap_f6_7`; o protótipo não compõe automaticamente todos os backends | Governança operacional integral e recovery F6.7 |
...
| Verificação e auditoria | F4.5 mantém a taxonomia única `typecheck/lint/unit_test/build/security_scan`, e runner `0/0` falham antes de subprocessos; F6.1–F6.3 fornecem journal/audit/evidence fail-closed; F6.5 adiciona inspeção e a F6.6 promovida documenta nove checkpoints | Proteção sem chave é somente “tamper-evident local”; a transação knowledge aguarda F6.7 | Matriz operacional integral com recovery ampliado |
...
- **Fase 5 e corretiva F5.C1 promovidas; reconciliação administrativa publicada:** a F5.1 foi promovida pelo [PR #53](https://github.com/Wf-ops1/Harnessinfra/pull/53).
  ...
  run `31963338576`. Sua reconciliação está `LOCAL_READY / PUBLICATION_PENDING`; a correção knowledge
  permanece reservada à F6.7.
