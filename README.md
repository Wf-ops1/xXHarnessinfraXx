...
| Configuração e governança | F5.1–F5.7, F5.C1 e F6.1–F6.4 estão promovidas; a reconciliação F6.4 encerrou no PR #76 e em `main` com 11/11 | A F6.5 está `COMPLETED_LOCAL / PROMOTION_PENDING`, com full/quality/package verdes; o protótipo não compõe automaticamente todos os backends | Governança operacional integral e recovery F6.6–F6.7 |
| CLI e scaffold | `--help`, `--version`, `init`, `compile`, `run`, `resume`, `approve`, `cancel`, `cleanup-worktree`, `rollback`, `list`, `status`, `inspect`, `events`, `evidence` e doctor possuem contratos/testes | Sem backends reais, `run` falha no preflight; os comandos F6.5 são inspeção local fail-closed e ainda não foram promovidos; estado/worktree válidos continuam necessários | UX estável para CLI e IDE em repositórios externos |
...
| Verificação e auditoria | F4.5 mantém a taxonomia única `typecheck/lint/unit_test/build/security_scan`, e runner `0/0` falham antes de subprocessos; F6.1–F6.3 promovidas fornecem journal/audit/evidence fail-closed; a F6.5 local adiciona status tipado, JSONL validado e verificação do manifesto | Proteção sem chave é somente “tamper-evident local”, não imutabilidade; F6.5 aguarda promoção e recovery amplo F6.6–F6.7 continua pendente | Matriz operacional integral com recovery ampliado |
...
- **Fase 5 e corretiva F5.C1 promovidas; reconciliação administrativa publicada:** a F5.1 foi promovida pelo [PR #53](https://github.com/Wf-ops1/Harnessinfra/pull/53).
  ...
  pós-merge `31931649225`. A F6.5 está `COMPLETED_LOCAL / PROMOTION_PENDING` somente em
  `task/f6.5-status-inspection`: `172` focados e full final `1023 passed, 5 skipped, 6 subtests`
  passaram, além de quality, build e smoke offline. F6.6–F6.7 não foram iniciadas.
