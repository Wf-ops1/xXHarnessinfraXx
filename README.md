...
| Verificação e auditoria | F4.5 mantém a taxonomia única `typecheck/lint/unit_test/build/security_scan`, e runner `0/0` falham antes de subprocessos; F4.6–F4.8 permanecem promovidas. F6.1 fornece `ExecutionEvent` 2.0; F6.2 promovida remove o writer paralelo, reutiliza lock/sequence/hash canônicos, detecta corrupção/gaps/duplicatas, oferece checkpoint HMAC opcional e export JSON/SARIF fail-closed com identidade exata | Proteção sem chave é somente “tamper-evident local”, não imutabilidade; composição automática/recovery continuam pendentes | Matriz integral fail-closed com repair/recovery integrada ao lifecycle padrão |
