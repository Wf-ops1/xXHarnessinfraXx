...
- **Fase 4 iniciada:** F3.1–F3.6, F3.8 e as corretivas F3.C1/F3.C2 aplicáveis foram
  ...
  também concluiu 11/11. O gate F4.4 foi congelado na branch `task/f4.4-typed-specific-plan` para
  substituir o plano genérico por contrato/structured output/persistência integrados ao lifecycle;
  sua implementação não foi iniciada. Nenhuma tag foi publicada, e F3.7 continua dependente da F4.7.
...
- [ContextAssembler](src/ai_engineering_harness/runtime/context_assembler.py) calcula as seis dimensões
  ...
  invariantes fail-closed R6 foram recertificadas e promovidas. O planner ainda fabrica plano genérico
  e não participa do lifecycle; o gate F4.4 está `READY`, mas a implementação não foi iniciada;
