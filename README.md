...
| Configuração e governança | F5.1–F5.7, F5.C1 e F6.1 estão promovidas; a reconciliação F6.1 encerrou no PR #70/merge `ac887b0`/CI `31888960272` 11/11 | O gate F6.2 está READY, mas produto, push e PR ainda não foram iniciados; o protótipo não compõe automaticamente todos os backends | Governança operacional integral e evidência/recovery F6 |
...
| Verificação e auditoria | F4.5 mantém a taxonomia única `typecheck/lint/unit_test/build/security_scan`; policy vazia/duplicada/desconhecida e runner `0/0` falham antes de subprocessos. F4.6–F4.8 permanecem promovidas. A F6.1 fornece um único `ExecutionEvent` 2.0, revalidação antes do hash, redaction tipada e aliases históricos resolvíveis; a R2 passou `325` testes focados e `935` no full | O gate F6.2 comprovou um writer legado paralelo, sem `execution_id`, lock ou sequência; hardening, HMAC opcional, export e semântica “tamper-evident local” estão congelados, ainda não implementados | Matriz integral fail-closed com repair/recovery integrada ao lifecycle padrão |
