...
| Configuração e governança | F5.1–F5.7, F5.C1, F6.1 e F6.2 estão promovidas; F6.3 está `PR_OPEN / CHECKS_PENDING` com manifest canônico, terminalização evidence-aware e recovery idempotente certificados | A F6.3 está publicada no PR #73, ainda sem promoção; o protótipo não compõe automaticamente todos os backends | Governança operacional integral e recovery/doctor F6 |
...
- **Fase 5 e corretiva F5.C1 promovidas; reconciliação administrativa publicada:** a F5.1 foi promovida pelo [PR #53](https://github.com/Wf-ops1/Harnessinfra/pull/53).
  ...
  `PR_OPEN / CHECKS_PENDING` na branch `task/f6.3-evidence-manifest`: `7` testes dedicados,
  `253` focados e a full R2 com `965 passed, 5 skipped, 6 subtests passed`; wheel e smoke offline
  estão verdes. O produto é `0bf3a59`, com o checkpoint `checkpoint/f6.3-complete`; a branch foi
  publicada no [PR #73](https://github.com/Wf-ops1/xXHarnessinfraXx/pull/73), cuja CI inicial
  `31913270722` está em andamento. Merge não foi autorizado. F6.4–F6.7 não foram iniciadas.
