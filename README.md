...
| Configuração e governança | F5.1–F5.7 e a corretiva F5.C1 estão promovidas; a reconciliação administrativa #68 também foi incorporada e certificada | A F6.1 está em reparo local após a revisão encontrar contratos knowledge ainda nomeados como eventos independentes; o protótipo não compõe automaticamente todos os backends | Governança operacional integral e evidência/recovery F6 |
...
- **Fase 5 e corretiva F5.C1 promovidas; reconciliação administrativa publicada:** a F5.1 foi promovida pelo [PR #53](https://github.com/Wf-ops1/Harnessinfra/pull/53).
  ...
  `task/f6.1-unified-event-schema`: embora `c9e41c4` tenha passado `282` testes focados e `929` no full,
  revisão posterior encontrou `KnowledgeSyncEvent` e `KnowledgeUpdateEvent` ainda registrados como
  modelos independentes. O estado voltou a `REPAIR_ACTIVE`; a R1 deve reclassificar esses contratos e
  recertificar antes de qualquer promoção. F6.2–F6.7 não foram declaradas entregues.
