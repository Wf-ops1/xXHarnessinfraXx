...
| Verificação e auditoria | F4.5 mantém a taxonomia única `typecheck/lint/unit_test/build/security_scan`; policy vazia/duplicada/desconhecida e runner `0/0` falham antes de subprocessos. A F4.6 promovida resolve a suíte no `ProvisionedWorktree`. A F4.7 persiste write-ahead/outcome e payloads commit-bound, recupera a suíte e impede `COMPLETED` sem ao menos um gate obrigatório executado e todos os obrigatórios aprovados | O PR #46 da F4.7 aguarda CI/promoção; reprovação permanece em `VERIFYING` porque repair/reexecução pertencem à F4.8 | Matriz integral fail-closed com repair/recovery operacional |
...
- **Fase 4 iniciada:** F3.1–F3.6, F3.8 e as corretivas F3.C1/F3.C2 aplicáveis foram
  ...
  isolada verdes; o PR #46 foi aberto e aguarda a matriz remota. A F4.8 não foi iniciada e F3.7 continua
...
- [ContextAssembler](src/ai_engineering_harness/runtime/context_assembler.py) calcula as seis dimensões
  ...
  PR #46 está aberto, enquanto CI e promoção ainda estão pendentes;
