...
| Configuração e governança | F5.1–F5.7, F5.C1 e F6.1–F6.4 estão promovidas; a reconciliação F6.4 encerrou no PR #76 e em `main` com 11/11 | O gate F6.5 está preparado localmente, mas a implementação ainda não foi autorizada; o protótipo não compõe automaticamente todos os backends | Governança operacional integral e recovery F6.5–F6.7 |
...
- **Fase 5 e corretiva F5.C1 promovidas; reconciliação administrativa publicada:** a F5.1 foi promovida pelo [PR #53](https://github.com/Wf-ops1/Harnessinfra/pull/53).
  ...
  pós-merge `31931649225`. A F6.5 possui gate `READY` local em
  `task/f6.5-status-inspection`, com baseline `139 passed in 28.52s`; implementação ainda não
  autorizada. F6.6–F6.7 não foram iniciadas.
