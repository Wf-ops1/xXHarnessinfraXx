...
| Configuração e governança | A F5.1 promovida resolve seis níveis por `importlib.resources`; a F5.2 promovida unifica policy default-deny e evidência durável nos oito eixos | A F5.3 está `READY / ACTIVE / LOCAL_ONLY` para tornar trust boundary uma decisão comum a imports, tools, worktree, hooks, promoção e leitura autorizada de secrets | Configuração e governança operacionais completas junto da conclusão da Fase 5 |
...
| Tool loop | A F5.2 promovida unifica a autorização em um engine tipado default-deny por role, node, workflow, trust mode, tool, operação, path e aprovação; o lote é pré-autorizado e a regra aplicada precede o efeito no journal. A factory F3.8 registra oito tools reais quando seus adapters são injetados | O gate F5.3 está congelado, mas a implementação ainda não integrou trust boundary a imports, comandos, worktree, hooks, promoção e secrets; aprovação vinculada ao conteúdo permanece F5.6 | Integração automática das tools e gates seguintes |
