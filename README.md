...
| Tool loop | A F5.2 promovida unifica a autorização em um engine tipado default-deny por role, node, workflow, trust mode, tool, operação, path e aprovação; o lote é pré-autorizado e a regra aplicada precede o efeito no journal. A F5.3 local exige o mesmo snapshot no router e nos adapters antes do efeito | A implementação F5.3 ainda é somente local; composição automática das tools não foi adicionada e aprovação vinculada ao conteúdo permanece F5.6 | Integração automática das tools e gates seguintes |
...
- **Fase 5 ativa:** a F5.1 foi promovida pelo [PR #53](https://github.com/Wf-ops1/Harnessinfra/pull/53).
  ...
  F5.3 está `COMPLETED_LOCAL / PROMOTION_PENDING / LOCAL_ONLY` em `task/f5.3-trust-boundary`. O focado
  ...
  Push, PR, merge, tags remotas e remoção de refs não estão autorizados.
