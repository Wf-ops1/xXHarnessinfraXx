...
| Configuração e governança | F5.1–F5.7, F5.C1 e F6.1–F6.5 estão promovidas no produto; F6.5 entrou em `main` pelo PR #77 com CI pós-merge verde | A reconciliação administrativa F6.5 ainda aguarda publicação; o protótipo não compõe automaticamente todos os backends | Governança operacional integral e recovery F6.6–F6.7 |
...
| Verificação e auditoria | F4.5 mantém a taxonomia única `typecheck/lint/unit_test/build/security_scan`, e runner `0/0` falham antes de subprocessos; F6.1–F6.3 fornecem journal/audit/evidence fail-closed; a F6.5 promovida adiciona status tipado, JSONL validado e verificação do manifesto | Proteção sem chave é somente “tamper-evident local”, não imutabilidade; recovery amplo F6.6–F6.7 continua pendente | Matriz operacional integral com recovery ampliado |
...
- **Fase 5 e corretiva F5.C1 promovidas; reconciliação administrativa publicada:** a F5.1 foi promovida pelo [PR #53](https://github.com/Wf-ops1/Harnessinfra/pull/53).
  ...
  `31953772121`. A reconciliação `docs/promote-f6.5` está local e aguarda publicação; F6.6–F6.7 não
  foram iniciadas.
