...
| Serena, índice, contexto e planejamento | Edição confinada e Serena MCP explícito usam efeitos verificados; `PythonAstIndexer` indexa o commit exato; a F4.3 calcula seis dimensões e aplica o dual gate; a F4.4 gera contrato tipado por structured output e persiste plano/eventos antes do grafo; a F4.C1 promoveu o claim concorrente exclusivo pelo PR #40 | Serena é opt-in; indexação é Python-only/full rebuild/explícita; a reconciliação administrativa F4.C1 está no PR #41 e ainda precisa ser incorporada antes da F4.5 | Backend Codebase-Memory compatível, memória semântica real e execução dos gates F4.5–F4.8 |
...
- **Fase 4 iniciada:** F3.1–F3.6, F3.8 e as corretivas F3.C1/F3.C2 aplicáveis foram
  ...
  administrativa foi publicada em `docs/promote-f4.c1` e está aberta no PR #41; o run inicial
  `31454615745` ainda não certifica o head final. A F4.5 permanece bloqueada até o merge administrativo
  e sua CI pós-merge verde. Nenhuma tag foi publicada, e F3.7 continua dependente da F4.7.
